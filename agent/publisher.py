...
(function() {
    ...
    function renderTopic(topic, idx) {
        ...
        if (searchQuery && visibleDevs.length === 0 && activeTags.size === 0 && activeRelevance === 'all') {
            // If searching and no devs match, check headline/summary too
            const q = searchQuery.toLowerCase();
    ...
    // Delegate tag clicks inside cards
...
def _get_archive_dates() -> list[str]:
...
def _archive_current(site_dir: Path) -> None:
    ...
    try:
        ...
        # Extract date from embedded DATA JSON
        import re
...
def publish(data_path: str | None = None) -> str:
